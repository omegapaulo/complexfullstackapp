...
export default class Search {
  ...
  keyPressHandler() {
    ...
    // If nothing is written in the search field
    if (value == '') {
    ...
    if (value != '' && value != this.previousValue) {
      // Clearing the search after every key press
      clearTimeout(this.typingWaitTimer);

      this.showLoaderIcon();
      this.hideResultsArea();

      // Waiting for some milliseconds before the searched info appears
  ...
  renderResultsHTML(posts) {
    if (posts.length) {
      ...
      this.resultArea.innerHTML = `<p class='alert alert-danger text-center shadow-sm'>Sorry, we can not find results for that search</p>`;
    }
    this.hideLoaderIcon();
  ...
  openOverlay() {
    ...
    // Waiting 50ms for the input field to be focused and user can type
    setTimeout(() => {
